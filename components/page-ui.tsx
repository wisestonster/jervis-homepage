...
export function ProductLanding({ product }: { product: Product }) {
  return <>
    ...
    <section className={`product-hero ${product.accent}`}><div className="container product-hero__grid"><div><p className="eyebrow">{product.status}</p><h1>{product.name}</h1><p className="product-category">{product.category}</p><h2>{product.tagline}</h2><p className="product-description">{product.description}</p><div className="hero-actions">{product.demo ? <a className="button" href={product.demo} target="_blank" rel="noreferrer">{product.demoLabel ?? "데모 서비스"} <Arrow /></a> : <span className="button button--disabled">COMING SOON</span>}<Link className="button button--secondary" href="/contact">도입 문의</Link></div></div>{product.posterImage ? <div className="product-poster"><Image src={product.posterImage} width={891} height={1270} alt={`${product.name} 포스터`} /></div> : <div className="product-console"><div className="console-top"><span>{product.name.toUpperCase()}</span><i /></div><div className="console-mark">{product.markImage ? <Image src={product.markImage} width={120} height={109} alt={`${product.name} 아이콘`} /> : product.name.slice(0, 2).toUpperCase()}</div><p>{product.category}</p><div className="console-status"><span>STATUS</span><strong>{product.demo ? "SERVICE READY" : "IN DEVELOPMENT"}</strong></div></div>}</div></section>
